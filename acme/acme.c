...
mousethread(void *v)
{
	...
	for(;;){
		...
		case MResize:
			...
			draw(screen, screen->r, backgroundcolor, nil, ZP);
			iconinit();
			scrlresize();
...
iconinit(void)
{
	...
	/* Shrub9 theme - cream backgrounds with golden dividers */
	tagcols[BACK] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xFFFFEAFF); /* cream background */
	tagcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xC99043FF); /* active golden */
	tagcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xDDBD8CFF); /* light golden dividers */
	tagcols[TEXT] = display->black;
	tagcols[HTEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xFFFFEAFF); /* cream text on golden */

	/* Text area - cream background */
	textcols[BACK] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xFFFFEAFF); /* cream background */
	textcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xC99043FF); /* active golden highlight */
	/* Halftone border pattern */
	textcols[BORD] = allocimage(display, Rect(0,0,2,2), CMAP8, 1, 0xDDBD8CFF);
	draw(textcols[BORD], Rect(1,1,2,2), textcols[BACK], nil, ZP); /* cream dots */
	draw(textcols[BORD], Rect(0,0,1,1), textcols[BACK], nil, ZP); /* cream dots */
	textcols[TEXT] = display->black;
	textcols[HTEXT] = display->black;

	/* Global divider and background colors */
	dividercolor = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xDDBD8CFF);
	backgroundcolor = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xFFFFEAFF);
	...
	/* Button - golden border with cream interior */
	button = allocimage(display, r, screen->chan, 0, DNofill);
	draw(button, r, tagcols[BORD], nil, r.min);
	r.max.x -= 4;
	fillellipse(button, (Point){r.min.x + 5, r.min.y + 7}, 3, 3, tagcols[BACK], ZP); /* cream circle */

	/* Mod Button - golden with black center */
	r = button->r;
	...
	r = button->r;
	colbutton = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xC99043FF); /* active golden */
	but2col = allocimage(display, r, screen->chan, 1, 0x000000FF); /* black for button 2 */
	but3col = allocimage(display, r, screen->chan, 1, 0xC99043FF); /* active golden for button 3 */
}
